...
def admin_chat_documents(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                chat_service.process_document(document)
                
                messages.success(request, _("Document uploaded and processed successfully."))
    ...
    # Get all documents
    documents = ChatDocument.objects.all().order_by('-uploaded_at')
    ...
    context = {
        'form': form,
        'page_obj': page_obj,
        'cancer_types': CancerType.objects.filter(is_organ=True),
        'status_filter': status_filter,
        'cancer_type_filter': cancer_type_filter,
    }

    return render(request, 'chat/admin/documents.html', context)
